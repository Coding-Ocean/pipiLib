...
void MATRIX::translate(FLOAT3& t)
{
...
void MATRIX::lookat(const FLOAT3& camPos, const FLOAT3& lookat, const FLOAT3& up) {
    //カメラのローカル軸座標を求める(正規直交ベクトル）
    FLOAT3 z = normalize(camPos - lookat);
...
void MATRIX::mulTranslate(const FLOAT3& t)
{
...
void MATRIX::mulScaling(const FLOAT3& s)
{
...
void MATRIX::rotateYXZ(const FLOAT3& r)
{
    rotateY(r.y);
    mulRotateX(r.x);
    mulRotateZ(r.z);
}
void MATRIX::mulRotateYXZ(const FLOAT3& r)
{
    mulRotateY(r.y);
    mulRotateX(r.x);
    mulRotateZ(r.z);
...
FLOAT3 MATRIX::operator*(const FLOAT3& v) const {
    FLOAT3 tmp;
